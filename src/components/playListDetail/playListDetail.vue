    ...
    <div class="playlist-info">
      <div class="info-wrapper">
        ...
        <div class="info-title">
          ...
          <p class="author">
            <mu-avatar solt="left" :src="creator.avatarUrl +
              '?param=50y50'" :size="30" :iconSize="20"/>
            <span>{{creator.nickname}}</span>
          </p>
        </div>
      </div>
      <div class="bg-mask"></div>
      <div class="bg-player" id="backImg" :style="{'background-image':
          'url(' + coverImgUrl + '?param=300y300'}"></div>
    </div>
    <div class="playlist-holder">
      <div class="add-all">
        <mu-flat-button label="播放全部" class="demo-flat-button"
                        icon="add_circle_outline" @click="playAll"/>
        <mu-divider/>
      </div>
      <div>
        <mu-circular-progress :size="40" class="center" v-if="isloading"/>
        <mu-list :value="value" v-show="!isloading"
                 @change="change">
          <div v-for="(item, index) in list" @click="playAudio(item)">
            <mu-list-item :disableRipple="true" :title="item.name"
                          :value="item.id" :describeText="item.ar[0].name">
